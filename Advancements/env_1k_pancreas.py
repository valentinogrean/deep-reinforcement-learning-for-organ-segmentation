...
def generate_slices_with_organ():
    ...
    with open("pickles\\patients_1k_organ4", "wb") as fp:   #Pickling
        pickle.dump(patients, fp)
    with open("pickles\\images_1k_organ4", "wb") as fp:   #Pickling
        pickle.dump(images, fp)
    with open("pickles\\segmentations_1k_organ4", "wb") as fp:   #Pickling
...
def get_full_segmentation(patient_number, slice_number):
    ...
    segmentation_slice[segmentation_slice == 4] = 1
...
class CTEnv1k_Pancreas:
    def __init__(self, render=False, final_render = False, random_slice=True, patient_number=0, slice_number=0):
        ...
        if random_slice == True:
            ...
            with open("pickles\\patients_1k_organ4", "rb") as fp:   # Unpickling
                self.patients = pickle.load(fp)
            with open("pickles\\images_1k_organ4", "rb") as fp:   # Unpickling
                self.images = pickle.load(fp)
            with open("pickles\\segmentations_1k_organ4", "rb") as fp:   # Unpickling
        ...
        self.new_range_start = 100
        self.new_min_end = 80
        self.new_range_end = 120
    ...
    def step(self, action):
        ...
        #if np.abs(self.y1_start - start_value) <= 15:
        #    reward += 0.1
        #if np.abs(self.y1_end - end_value) <= 15:
        #    reward += 0.1

        if start_value > end_value:
            reward -= 0.4
        
        #reward += (0.9 ** np.abs(ground_truth_start - start_value)) * 0.4
        #reward += (0.9 ** np.abs(ground_truth_end - end_value)) * 0.4
        ...
        #update own segmentation - line
        #rr, cc = draw.line(self.x, start_value-2, self.x, start_value+2)
        #self.own_segmentation[rr, cc] = 0.9
        #rr, cc = draw.line(self.x, end_value-2, self.x, end_value+2)
        #self.own_segmentation[rr, cc] = organ_segmentation_value
